...
function Add_AE(name, property) {
	...
	this.create = function () {
		...
		if (comp != null && comp instanceof CompItem) {
			app.beginUndoGroup("Отмена: Создать Еффект");
				...
				if (layers){
				    ...
				    try{
					    ...
					    for (i = 0; i < prop.length; i++){
					          ...
					          properties[i].expression = _exp;
					    };
					} catch(err){};
				    app.endUndoGroup();
				};
			} else {
				xAlert("Необходимо выделить композицию");
			}
		}
	} catch (err) {}
};
...
function newSolid() {
	...
	if (activeComp instanceof CompItem) {
		...
		xAlert("Необходимо выбрать композицию!");
	}
...
function newAdjustment() {
	var activeComp = app.project.activeItem;
	if (activeComp instanceof CompItem) {
		var solid = activeComp.layers.addSolid([1,1,1], "Adjustment Layer", activeComp.width, activeComp.height, activeComp.pixelAspect, activeComp.duration);
...
function newcamera() {
	...
	if (comp instanceof CompItem) {
		...
		cameraLayer.transform.position.setValue([960,540,-1866.6667]);
	}
...
var addElement = function (){
	...
	if (comp instanceof CompItem) {
		...
		var newElementFX = new Add_AE("Element", "VIDEOCOPILOT 3DArray", false);
		newElementFX.create();
		if (selLayer[0]) {
			var NELayer = comp.layer("Element");
			NELayer.effect(1)("VIDEOCOPILOT 3DArray-1802").setValue(selLayer[0].index);
			selLayer[0].enabled = false;
		} else {
...
var setWiggle = function (){
	var comp = app.project.activeItem;
	if (comp instanceof CompItem) {
		var exp = wiggle_exp();
		if (exp != null) {
			var newExpression = new SetExpression(exp);
